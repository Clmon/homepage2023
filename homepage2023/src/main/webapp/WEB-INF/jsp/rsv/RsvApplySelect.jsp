...
<body>
	...
	<div id="content">
		<div class="container">
			<div id="contents">
				<div class="bbs_list">
					<form action="/rsv/rsvApplyUpdate.do" method="post" id="frm"
						name="frm" onsubmit="return regist()">
								...
								<c:choose>
									<c:when test="${result.confmSeCode eq 'R' }">
										<tr>
											...
											<td><input type="text" id="chargerNm" name="chargerNm"
												title="예약자명" value="<c:out value="${result.chargerNm }" />" />
											</td>
										</tr>
										<tr>
											<th scope="row">* 연락처</th>
											<td><input type="text" id="telno" name="telno"
												title="연락처" value="<c:out value="${result.telno}" />" /></td>
										</tr>
										<tr>
											<th scope="row">이메일</th>
											<td><input type="text" id="email" name="email"
												" title="이메일" value="<c:out value="${result.email }" />" />
											</td>
										</tr>
									</c:when>
									<c:otherwise>
										<tr>
											...
											<td><c:out value="${result.chargerNm}" /> (<c:out
													value="${result.frstRegisterId }" />)</td>
										</tr>
										<tr>
											<th scope="row">연락처</th>
						...
						<div class="btn-cont ar">
							...
							<c:if test="${result.confmSeCode eq 'R'}">
								...
								<a href="${delUrl }" id="btn-del" class="btn"><i
									class="ico-del"></i> 삭제</a>
							</c:if>
							<c:url var="listUrl"
								value="/rsv/selectApplyList.do${_BASE_PARAM}" />
